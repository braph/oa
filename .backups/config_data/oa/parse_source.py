...
Usage:
$ ./parse_source.py /path/to/OA/source/**/*.c
'''
...
DEFAULT     = 2
FLAGS       = 3
TYPE2STR    = {int:'int', float:'float', bool:'bool', None:'None'}
IDENTIFIER  = r'\w[\w\d_]+'
WS          = re.compile(r'\s+')
RE_IGN_WHITESPACES = lambda p: re.compile(WS.sub(r'\\s*', p), re.VERBOSE)

is_float = re.compile('\d+\.\d+').fullmatch
...
class VariableDef:
    ...
    def __repr__(self):
        return "{'name': %r, 'default': %r, 'cheat': %r, 'type': %s}" % (
            self.name, self.default, self.cheat, TYPE2STR[self.type])
...
#static cvarTable_t gameCvarTable[] = {
    ...
    for line in fh:
        ...
        if vals:
            if 'CVAR_ROM' in vals[FLAGS]:
                continue

            cheat = 'CVAR_CHEAT' in vals[FLAGS]
            yield VariableDef(
                vals[SOURCE_NAME].lstrip('&'),
                vals[VARNAME].strip('"'),
                vals[DEFAULT].strip('"'),
                cheat, None)
...
# CVars have been collected, now check for type
...
print('# This file contains variable definitions taken from C source code.')
print('# It has been generated by', sys.argv[0])
print('LABEL = %r' % 'Source')
